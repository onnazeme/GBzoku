/*:
 * @plugindesc v.5.5.8 [MV] Localization of game.
 * @author DKPlugins
 ...
 * @help
 ...
 4. Wrap all dialog text in tags {}: WrapDialogText
 The plugin command checks json map files from the "data/" folder and wraps them in tags
 all dialogue texts (Show text and Show choices).
 In the plugin command parameters you can specify which maps to update.
 You can select all maps, the current map, or specify the ID of a specific map.
 It is recommended to backup your files before using the command.
 After executing the command, you must reload the project.
...
/*:ru
 * @plugindesc v.5.5.8 [MV] Локализация игры.
 * @author DKPlugins
 * @url https://dk-plugins.ru
 * @target MV
 * @base DKTools
 ...
 * @help
 ...
 4. Обернуть тексты диалогов в теги {} : WrapDialogText
 Команда плагина проверяет json файлы карт из папки "data/" и оборачивает в теги
 все тексты диалогов (Показать текст и Показать выбор).
 В параметрах команды плагина вы можете указать в каких картах сделать обновление.
 Вы можете выбрать все карты, текущую или указать ID определенной карты.
 Рекомендуется сделать бэкап файлов перед использованием команды.
 После выполнения команды необходимо перезагрузить проект.
...
Imported['DKTools_Localization'] = '5.5.8';
...
DKTools.PluginCommandManager.set('WrapDialogText', async (args) => {
    if (!Utils.isNwjs()) {
        return;
    }

    if (!window.confirm('Are you sure you want to wrap dialog text? It is recommended to backup your files!')) {
        return;
    }
    ...
    if (args.map === 'Current map') {
        args.map = $gameMap.mapId();
    }

    switch (args.map) {
        case 'All maps':
            files = _.filter(result.data,
                file => file.getName().match(/Map\d{3,}/));
            break;
        default:
            files = _.filter(result.data, file => {
                const match = file.getName().match(/Map\d{3,}/);

                return match && parseInt(match[0].substring(3)) === parseInt(args.map);
            });
            break;
    }
                    ...
                    if (page.list) {
                        page.list.forEach((command) => {
                            if (command.code === 401) { // Show text
                                const result = processEventText(command.parameters[0]);

                                if (result.success) {
                                    command.parameters[0] = result.text;
                                    updated++;
                                }
                            } else if (command.code === 102) { // Show choices
                                let success = false;

                                command.parameters[0].forEach(((text, index) => {
                                    const result = processEventText(text);

                                    if (result.success) {
                                        command.parameters[0][index] = result.text;
                                        success = true;
                                    }
                                }));
    ...
    files.forEach(async (file) => {
        ...
        if (result.status !== DKTools.IO.OK) {
            console.error(`Cannot load file ${file.getFullName()}`);

            return;
        }

        const map = result.data;
        const updatedStrings = processMapEvents(map.events);

        if (updatedStrings === 0) {
            return;
        }

        const saveResult = await file.saveJsonAsync(map);

        if (saveResult === DKTools.IO.OK) {
            console.info(`File ${file.getFullName()} was saved. Updated ${updatedStrings} strings`);
            updatedFiles++;
        } else {
            console.error(`Cannot save file ${file.getFullName()}`);
        }
    });

    if (updatedFiles > 0) {
        alert(`Updated ${updatedFiles} files! Reload the RPG Maker program!`);
    } else {
        alert('No files were updated! Check the plugin command parameters.');
    }
});
